...
3. Try to click remove link on my cart contents for one item. It doesn't work (see next on how to remove it incorrectly)
  ...
  ```
4. If -1 is set in one item, all items are removed!, but…
5. Try to add a new item product, the shopping cart reappears!
6. We set 0 and press update. The price item still is shown in cart!
7. Total price is round! (shopping cart box)
8. Cart contents top button has a spelling mistake
